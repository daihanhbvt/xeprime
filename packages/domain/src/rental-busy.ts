 ...
 *
 * Lấy thẳng từ OpenAPI (ADR 0007) chứ không alias qua `apps/web/src/features/booking-requests`
 * như bản cũ: package dùng chung không được biết `apps/web` tồn tại, và shape này là hợp đồng
 * của endpoint chứ không phải kiểu riêng của một màn hình web.
 */
export type VehicleBusyDay = components['schemas']['VehicleBusyDayDto'];
