...
The same spec is what another engineer reads at `/docs`, so declaring it is not paperwork. Three things are yours to write; everything else is generated. The controller carries `@ApiTags` with a name that already exists in `openapi/api-tags.ts` — a new area means adding it there with a description, not inventing a loose tag. Every route carries `@ApiOperation({ summary })`. Every route declares its success response with `type:` pointing at a named DTO — never an inline `schema: { properties: ... }`, which generates an anonymous shape the frontend cannot name; for the small ad-hoc results reach for `OkResultDto` / `IdResultDto` / `DeletedCountDto` in `common/dto/api-response.dto.ts` instead of a fresh one-off. The `{ data }` envelope, the security lock, the required permissions, and the 400/401/403/404/409/429/500 branches are derived from your guard metadata — do not hand-write them. `test/openapi-contract.spec.ts` fails with the exact offending routes if any of the three are missing; `docs/api-docs.md` is the full picture.
