...
describe('nav — cấu trúc cây', () => {
  it('gian hàng: 1 mục gốc + 2 nhóm, tổng 15 mục lá', () => {
...
describe('nav — vai trò gian hàng nhìn thấy gì', () => {
  it('shop_owner thấy đủ 15 mục', () => {
    expect(
      visibleLabels(DEFAULT_TENANT_ROLE_PERMISSIONS[TENANT_ROLE.SHOP_OWNER], false),
    ).toHaveLength(15);
  });

  it('shop_manager cũng thấy đủ 15 mục (có MEMBER_VIEW và FINANCE_VIEW)', () => {
    expect(
...
describe('nav — vai trò nền tảng nhìn thấy gì', () => {
  it('platform_admin thấy đủ 9 mục', () => {
    expect(
