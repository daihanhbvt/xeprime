...
import { CUSTOMERS_DEFAULT_LIMIT, duplicateCustomerId, filtersToParams } from './api';

/**
 * Bộ lọc sổ khách của WEB.
 *
 * ADR 0031: app native serialize bộ lọc này bằng bản của nó
 * (`apps/mobile/src/api/customers/api.ts`, cùng bộ test). Hai client dựng hai bộ query params
 * khác nhau cho cùng một endpoint là cách chắc chắn nhất để một `invalidateQueries` chỉ làm mới
 * đúng một nửa số màn, và để một bên im lặng gửi thiếu tham số — nên hai bộ test này phải giữ
 * cùng kỳ vọng.
 */
describe('filtersToParams', () => {
  ...
  it('trường không đặt thành `null`, KHÔNG thành chuỗi rỗng', () => {
    const params = filtersToParams({ q: 'an' });
  ...
  it('giữ nguyên trang và limit người gọi truyền vào', () => {
    expect(filtersToParams({ page: 3, limit: 10 })).toMatchObject({ page: 3, limit: 10 });
  });
