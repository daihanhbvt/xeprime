 ...
 * hàm mà backend dùng để dựng chuỗi lưu xuống DB — nên thứ người dùng đọc ở bước Xác nhận là
 * đúng thứ sẽ được lưu. Viết một phép nối chuỗi riêng ở màn xác nhận là cách chắc chắn để hai
 * bên lệch nhau đúng vào lúc người dùng đang soát lại lần cuối.
 *
 * Tên tỉnh/xã lấy từ hai query danh mục ĐÃ CÓ SẴN trong cache (staleTime 30 phút, và ô nhập
 * địa chỉ vừa tải chúng xong), nên hook này không sinh thêm request nào.
 *
 * Trả `null` khi chưa đủ dữ liệu — nơi gọi hiện dấu gạch thay vì một chuỗi ghép dở.
 */
