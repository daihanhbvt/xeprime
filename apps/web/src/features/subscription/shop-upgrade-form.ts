...
import type { Branch, UpdateBranchInput } from '@/features/branches/types';
import { toShopProfileBody, toShopProfileValues } from '@/features/shop/shop-profile-form';
import type { MyShop, UpdateProfileInput } from '@/features/shop/types';
 ...
 *
 * ## Vì sao đi qua `toShopProfileValues` / `toShopProfileBody`
 *
 ...
 * những ô người dùng vừa sửa, bằng chính hai hàm mà trang Cửa hàng và màn đăng ký đang dùng.
 */
...
export function toShopUpgradeValues(shop: MyShop, branch: Branch | null): ShopUpgradeValues {
  const profile = toShopProfileValues(shop);
  return {
    displayName: profile.displayName,
    /*
     * Chi nhánh đọc TRƯỚC hồ sơ ở cả ba ô địa chỉ: hai cột tỉnh trên `tenant_profiles` chỉ là bản
     * sao (`syncProfileFromDefaultBranch`), và `addressLine` của chi nhánh là giá trị THẬT — còn
     * `toShopProfileValues` chỉ ĐOÁN nó ra từ chuỗi hiển thị đã ghép.
     */
    provinceCode: branch?.provinceCode ?? profile.provinceCode,
    wardCode: branch?.wardCode ?? profile.wardCode,
    addressLine: branch?.addressLine ?? profile.addressLine,
...
export function toShopUpgradeProfileBody(
  ...
  return toShopProfileBody({
    ...toShopProfileValues(shop),
