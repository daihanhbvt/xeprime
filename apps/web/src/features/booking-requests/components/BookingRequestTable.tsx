...
interface Props {
  ...
  /** Yêu cầu dài hạn cần chốt lịch → trang mở hộp thoại; dịch vụ khác duyệt thẳng. */
  onApprove: (row: BookingRequestItem) => void;
...
export function BookingRequestTable({
  ...
  const columns: DataTableColumn<BookingRequestItem>[] = [
    ...
    {
      /*
       * Yêu cầu THUÊ DÀI HẠN chưa duyệt KHÔNG có lịch (ADR 0011) — cột này nói GÓI và NGUYỆN
       * VỌNG ngày nhận. Bịa một khoảng ngày ở đây sẽ khiến chủ xe tưởng khách đã chốt lịch.
       */
    ...
    },
    {
      ...
      render: (_, row) => (
        <div>
          ...
          {row.serviceType === SERVICE_TYPE.LONG_TERM && row.longTermPackageMonths ? (
            <Tag>{longTermPackageLabel(row.longTermPackageMonths)}</Tag>
          ) : null}
          ...
          ? [
              {
                ...
                // Dài hạn mở hộp thoại CHỐT LỊCH (không confirm nhanh): gian hàng phải chọn
                // ngày giờ nhận chính xác trước khi đơn được tạo.
                confirm:
