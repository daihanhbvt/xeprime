...
Thường không cần cấu hình gì: [src/lib/api-base-url.ts](src/lib/api-base-url.ts) suy host từ Expo
dev server — thiết bị thật lấy IP LAN của máy chạy Metro, emulator Android dùng `10.0.2.2`.

Host được dò qua **bốn nguồn** chứ không một: `expoConfig.hostUri` → `expoGoConfig.debuggerHost`
→ `experienceUrl` → `linkingUri`. Bản Expo Go cũ hơn SDK của dự án **không điền `hostUri`**, và
chỉ đọc một nguồn thì tất cả rơi về loopback → `10.0.2.2` → trên máy thật đó là địa chỉ không
tồn tại, mọi request treo tới hết trần 15 giây rồi báo "Máy chủ phản hồi quá lâu" — trông y hệt
server chết. Không dò được nguồn nào thì app **kêu to trong log**, không lặng lẽ đoán.

Khi log lúc khởi động in ra host sai, hoặc API không nằm ở cổng 4000 của chính máy chạy Metro,
đặt `EXPO_PUBLIC_API_URL` trong `apps/mobile/.env`:

```
EXPO_PUBLIC_API_URL="http://192.168.1.183:4000"
```

Expo chỉ inline biến có tiền tố `EXPO_PUBLIC_`, và inline **lúc build** — đổi giá trị phải khởi
động lại Metro.

### Khi request treo

Mọi request đều được ghi log ở bản dev ([fetch-with-logging.ts](src/lib/fetch-with-logging.ts)),
và **thời gian đã đợi tự chẩn đoán nguyên nhân**:

| Log | Nghĩa là | Sửa ở đâu |
| --- | --- | --- |
| `API baseUrl: http://10.0.2.2:4000` trên **máy thật** | Không đọc được host dev server | Đặt `EXPO_PUBLIC_API_URL` |
| `✕ … (15003ms)` | Gói bị **thả rơi** | Firewall máy dev chặn cổng 4000 · điện thoại khác mạng (4G thay vì wifi) · router bật AP isolation |
| `✕ … (12ms)` | Bị **từ chối** thẳng | API chưa chạy, hoặc sai cổng |
| `← 401 … (180ms)` | Mạng thông | Lỗi thật của request |

Kiểm tra nhanh nhất: mở `http://<IP máy dev>:4000/health` bằng trình duyệt **trên chính điện
thoại đó**. Ra JSON là mạng thông.

Log chỉ ghi metadata — không header, không body, không response. `Authorization` mang access
token, body đăng nhập mang mật khẩu, response mang refresh token (ADR 0017).
