 ...
 * nữa. Không có nó, một lỗi mạng ở bước lưu giá sẽ đẻ ra chiếc xe thứ hai mỗi lần người dùng bấm
 * "Thử lại" — và chủ xe phải tự dọn.
 */
  ...
  const run = useCallback(
    async (
      ...
      try {
        ...
        if (!vehicle) {
          vehicle = await vehiclesApi.create(quickVehicleToCreateInput(values));
