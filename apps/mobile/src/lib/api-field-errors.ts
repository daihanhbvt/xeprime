...
/**
 * Một lỗi cấp TRƯỜNG do backend trả về — bản native của `apps/web/src/lib/api-field-errors.ts`.
 *
 ...
 *
 * `detail` là câu kỹ thuật của backend (tiếng Việt, hoặc chuỗi tự sinh của class-validator). Nó
 * KHÔNG dùng để hiện lên màn hình — ADR 0012: chữ cho người dùng đi từ mã lỗi, không từ `message`
 * của backend. Giữ lại vì nó là thứ đáng ghi vào console khi lần dấu.
 */
...
function issueFrom(entry: unknown): ApiFieldIssue | null {
  ...
  return detail === undefined ? { field } : { field, detail };
}

/**
 * Rút danh sách lỗi cấp trường ra khỏi một lỗi API — rỗng nếu lỗi này không phải loại đó.
 *
 * Backend phát `details` ở HAI hình dạng: mảng `[{ field, constraints }]` do pipe validate toàn
 * cục sinh, và `{ fields: [{ field, message }] }` do vài chỗ validate viết tay sinh. Nơi gọi không
 * nên phải biết mình đang nhận cái nào, nên chỗ duy nhất biết điều đó là hàm này.
 */
