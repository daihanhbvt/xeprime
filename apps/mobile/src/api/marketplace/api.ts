...
export const marketplaceApi = {
  ...
  /**
   * Xe công khai CỦA MỘT gian hàng.
   *
   * Endpoint riêng chứ không phải `/public/listings?shop=…`: danh sách này bỏ qua bộ lọc chợ và
   * luôn trả đúng đội xe đang hiển thị của gian hàng đó.
   */
  shopListings(
    slug: string,
    params: { page: number; limit: number },
    signal?: AbortSignalLike,
  ): Promise<Paged<PublicListing>> {
    return getApiClient().fetchPage<PublicListing>(
