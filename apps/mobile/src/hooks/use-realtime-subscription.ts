 ...
 *
 * Bản native của `apps/web/src/hooks/use-realtime-subscription.ts`, cùng máy trạng thái và cùng
 * luật thử lại. Chép sang thay vì đặt ở `@xeprime/domain` vì nó là một React hook — package đó
 * framework-free có chủ đích (CLAUDE.md §3).
 *
 * Lỗi đã gặp thật, và mobile đang mắc ĐÚNG nó: nhịp poll của thread chọn theo `ready` của
 * `ChatRealtimeProvider`, mà `ready` chỉ chứng minh `signInWithCustomToken` thành công. Đăng nhập
 * được Firebase và LẮNG NGHE được một document là hai chuyện khác nhau — rules chưa đẩy, rules
 * sai, hoặc subscription chết giữa chừng đều để `ready = true` trong khi không snapshot nào bao
 * giờ tới. Client tin mình đang realtime nên hạ nhịp hỏi lại xuống mức lưới an toàn, và người
 * nhận phải đợi trọn một nhịp đó mới thấy tin nhắn.
 *
...
export const REALTIME_STATE = {
  ...
  /** Đã nhận snapshot từ SERVER — điều kiện DUY NHẤT để tin là realtime đang chạy. */
  LIVE: 'live',
...
export interface RealtimeHandlers {
   ...
   * minh đường ống phía sau còn sống — nó chỉ chứng minh máy còn nhớ dữ liệu cũ. Nơi gọi phải tự
   * lọc bằng `snapshot.metadata.fromCache` trước khi gọi hàm này.
   */
  live(): void;
...
/**
 * Nhiễu ±25%. Không có nó thì mọi máy cùng rớt kết nối sẽ cùng thử lại đúng một khoảnh khắc, và
 * Firestore nhận một đợt dồn thay vì một dòng đều.
 */
...
export interface RealtimeSubscriptionOptions {
  ...
  enabled: boolean;
  /**
   * Danh tính của subscription. Đổi khoá (đổi hội thoại, đổi tài khoản) ⇒ tháo listener cũ, dựng
   * listener mới, và VỨT trạng thái cũ — không để một thread báo "live" nhờ thread trước đó.
   */
  key: string;
...
export function useRealtimeSubscription(options: RealtimeSubscriptionOptions): RealtimeState {
  ...
  /*
   * `subscribe` gần như luôn là một closure mới mỗi lần render. Để nó trong deps nghĩa là tháo và
   * dựng lại listener sau MỌI lần render — vừa tốn, vừa làm trạng thái nhấp nháy. Giữ bản mới
  ...
  useEffect(() => {
    ...
    return () => {
       ...
       * `ChatRealtimeProvider` đặt `ready = false` ở cleanup mỗi khi danh tính người dùng đổi rồi
 ...
 *
 * Ở production im lặng là đúng: đường dự phòng đã lo phần chức năng, và người dùng không làm gì
 * được với "permission-denied" của Firestore. Nhưng im lặng ở dev chính là thứ khiến lỗi này sống
 * sót lâu đến vậy — mọi tầng đều trông như đang chạy, chỉ có độ trễ là sai.
 *
 * Không bao giờ log token, dữ liệu người dùng hay nội dung snapshot: chỉ nhãn và mã lỗi đã lọc.
 */
function warnRealtime(label: string, error: unknown): void {
  if (!__DEV__) return;
  chatDebug.realtimeFailed(label, error);
}
