...
| command | `/commit` | Người dùng gõ tay sau khi review — chạy trọn quy trình Git: cập nhật `develop` → tạo branch → `add -A` → commit (KHÔNG push/merge). Quy ước ở `docs/git-workflow.md` |
